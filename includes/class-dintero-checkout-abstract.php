<?php
/**
 * Dintero Checkout Abstract Class Doc Comment
 *
 * @category Dintero_Checkout_Abstract
 * @package  Dintero
 * @author   Dintero
 */

/**
 * Dintero Checkout Abstract
 */
abstract class Dintero_Checkout_Abstract
	 ...
	 * Instantiating class
	 *
	 * @param WC_Payment_Gateway $payment_gateway payment gateway class.
	 * @param Dintero_Adapter    $adapter adapter class.
	...
	public function capture() {
		...
		$error       = $this->helper()->extract( 'error', $transaction );
		if ( $error ) {
	...
	public function refund( $amount = null ) {
		...
		$response = $this->adapter->refund_transaction(
			$this->order->get_transaction_id(),
			array(
				'amount' => $amount,
				'items'  => $this->helper()->prepare_items_from_order( $this->order ),
